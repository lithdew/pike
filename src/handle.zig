...
pub const Handle = struct {
    ...
    pub inline fn fromOverlapped(overlapped: windows.LPOVERLAPPED) *Self {
        return @fieldParentPtr(Self, "waker", @fieldParentPtr(Waker, "data", @fieldParentPtr(Waker.Data, "request", overlapped)));
    }
...
test "Waker.wait" {
    ...
    var ptr = Waker.recover(waker.writers);

    testing.expect(ptr != null);
    testing.expect(ptr.?.tail != null);

    var a = Waker.recover(waker.writers);
    var b = a.?.next;
    var c = b.?.next;
    var d = c.?.next;

    testing.expect(waker.writers != @ptrToInt(@as(?*Waker.Node, null)));
    ...
    testing.expect(waker.writers & Waker.IS_READY != 0);
